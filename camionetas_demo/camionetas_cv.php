...
function mantenimiento_base_km(PDO $pdo, array $t): int {
    ...
    $stmt = $pdo->prepare("SELECT kilometraje FROM cv_camioneta_mantenimientos WHERE camioneta_id=? AND kilometraje IS NOT NULL ORDER BY fecha_mantenimiento DESC, id DESC LIMIT 1");
    $stmt->execute([$id]);
    ...
    $stmt = $pdo->prepare("SELECT kilometraje FROM cv_camioneta_indicadores_diarios WHERE camioneta_id=? ORDER BY fecha ASC, id ASC LIMIT 1");
    $stmt->execute([$id]);
...
/* =========================================================
   MODO CV / DEMOSTRACIÓN
   No usa las tablas reales del sistema. Esta copia trabaja con
   tablas demo prefijadas cv_ y con datos ficticios.
========================================================= */
$rolSesion = 'demo';
$puestoSesion = 'demo';
$perfilPermisos = 'demo';
...
/* =========================================================
   TABLAS DEMO PARA CURRÍCULUM
   Se crean solas con prefijo cv_ para no tocar las tablas reales.
========================================================= */
function cv_demo_init(PDO $pdo): void {
    $pdo->exec("CREATE TABLE IF NOT EXISTS cv_camionetas (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        eco TEXT NOT NULL UNIQUE,
        modelo TEXT NOT NULL,
        no_serie TEXT NOT NULL UNIQUE,
        matricula TEXT NOT NULL UNIQUE,
        actividad TEXT NOT NULL DEFAULT 'sin uso',
        fecha_ultimo_mantenimiento TEXT NULL,
        kilometraje_actual REAL NOT NULL DEFAULT 0,
        tanque_gasolina REAL NOT NULL DEFAULT 0,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
    )");

    $pdo->exec("CREATE TABLE IF NOT EXISTS cv_camioneta_conductores (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        FOREIGN KEY (camioneta_id) REFERENCES cv_camionetas(id) ON DELETE CASCADE
    )");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_cv_conductores_camioneta ON cv_camioneta_conductores (camioneta_id)");

    $pdo->exec("CREATE TABLE IF NOT EXISTS cv_camioneta_indicadores_diarios (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        camioneta_id INTEGER NOT NULL,
        fecha TEXT NOT NULL,
        kilometraje REAL NOT NULL DEFAULT 0,
        tanque_gasolina REAL NOT NULL DEFAULT 0,
        gasolina_depositada REAL NOT NULL DEFAULT 0,
        kilometraje_llegada REAL NULL,
        tanque_llegada_cedis REAL NULL,
        gasolina_depositada_llegada REAL NOT NULL DEFAULT 0,
        UNIQUE (camioneta_id, fecha),
        FOREIGN KEY (camioneta_id) REFERENCES cv_camionetas(id) ON DELETE CASCADE
    )");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_cv_indicadores_fecha ON cv_camioneta_indicadores_diarios (fecha)");

    $pdo->exec("CREATE TABLE IF NOT EXISTS cv_camioneta_mantenimientos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        camioneta_id INTEGER NOT NULL,
        fecha_mantenimiento TEXT NOT NULL,
        kilometraje REAL NULL,
        notas TEXT NULL,
        FOREIGN KEY (camioneta_id) REFERENCES cv_camionetas(id) ON DELETE CASCADE
    )");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_cv_mant_camioneta ON cv_camioneta_mantenimientos (camioneta_id)");

    $pdo->exec("CREATE TABLE IF NOT EXISTS cv_camioneta_compras (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        proveedor TEXT NULL,
        costo REAL NULL,
        estatus TEXT NOT NULL DEFAULT 'orden',
        notas TEXT NULL,
        fecha_creacion TEXT DEFAULT CURRENT_TIMESTAMP
    )");

    $total = (int)$pdo->query("SELECT COUNT(*) FROM cv_camionetas")->fetchColumn();
    if ($total > 0) return;

    $hoy = new DateTime('now', new DateTimeZone('America/Monterrey'));
    $nombres = ['Alex Torres','Diego Ruiz','Marco Rivera','Jesus Chuy','Juan Carlos','Santiago','Luis Eduardo','Carmelo','Humberto','Jonathan','Gilberto','Brayan','Felipe','Raul Medina'];
    $modelos = ['2023 NP300','2022 Frontier','2021 NP300','2020 Transit','2019 Sprinter','2024 NP300','2018 Frontier','2022 Partner','2021 NP300','2020 NP300','2023 Hilux','2019 Frontier','2022 NP300','2021 Transit'];
    $activas = [true,true,true,true,true,true,true,true,true,true,true,true,false,false];

    $insTruck = $pdo->prepare("INSERT INTO cv_camionetas (eco,modelo,no_serie,matricula,actividad,fecha_ultimo_mantenimiento,kilometraje_actual,tanque_gasolina) VALUES (?,?,?,?,?,?,?,?)");
    $insCond = $pdo->prepare("INSERT INTO cv_camioneta_conductores (camioneta_id,nombre,telefono,correo,ruta,fecha_inicio,activo) VALUES (?,?,?,?,?,?,1)");
    $insInd = $pdo->prepare("INSERT INTO cv_camioneta_indicadores_diarios (camioneta_id,fecha,kilometraje,tanque_gasolina,gasolina_depositada,kilometraje_llegada,tanque_llegada_cedis,gasolina_depositada_llegada) VALUES (?,?,?,?,?,?,?,?)");
    $insMant = $pdo->prepare("INSERT INTO cv_camioneta_mantenimientos (camioneta_id,fecha_mantenimiento,kilometraje,notas) VALUES (?,?,?,?)");
    ...
    for ($i=0; $i<14; $i++) {
        ...
        $ultimoKm = $baseKm;
        $ultimoGas = 40 + (($i * 7) % 45);
        $actividad = $activas[$i] ? 'en ruta' : 'sin uso';
        $fechaMant = (clone $hoy)->modify('-'.(18 + $i).' days')->format('Y-m-d');
        $insTruck->execute([$eco, $modelos[$i], 'CV-DEMO-SERIE-'.($i+1001), 'CV-'.str_pad((string)($i+11),3,'0',STR_PAD_LEFT), $actividad, $fechaMant, $ultimoKm, $ultimoGas]);
        $truckId = (int)$pdo->lastInsertId();
        if ($activas[$i]) {
            $insCond->execute([$truckId, $nombres[$i], '81 '.rand(1000,9999).' '.rand(1000,9999), strtolower(str_replace(' ', '.', $nombres[$i])).'@demo.local', 'Ruta '.($i+1), (clone $hoy)->modify('-20 days')->format('Y-m-d')]);
        }

        for ($d=13; $d>=0; $d--) {
            $fecha = (clone $hoy)->modify('-'.$d.' days')->format('Y-m-d');
            $salida = $baseKm + ((13-$d) * (62 + ($i % 5) * 11));
            $recorrido = 52 + (($i+$d) % 6) * 14;
            $llegada = $salida + $recorrido;
            $gasSalida = max(8, min(100, 82 - (($d + $i * 3) % 55) + (($i % 3) * .5)));
            $gasLlegada = max(5, $gasSalida - (8 + (($i+$d) % 10)));
            $depositadoSalida = (($i+$d) % 4 === 0) ? (350 + (($i+$d) * 25)) : 0;
            $depositadoLlegada = (($i+$d) % 7 === 0) ? (250 + (($i+$d) * 18)) : 0;
            if ($d === 0 && $activas[$i] && in_array($i, [3,6,11], true)) {
                $llegada = null;
                $gasLlegada = null;
                $depositadoLlegada = 0;
            }
            ...
            $insInd->execute([$truckId, $fecha, $salida, $gasSalida, $depositadoSalida, $llegada, $gasLlegada, $depositadoLlegada]);
            $ultimoKm = $llegada ?: $salida;
            $ultimoGas = $gasLlegada ?: $gasSalida;
        }

        $pdo->prepare("UPDATE cv_camionetas SET kilometraje_actual=?, tanque_gasolina=? WHERE id=?")->execute([$ultimoKm, $ultimoGas, $truckId]);
        if ($i % 4 === 0) {
            $insMant->execute([$truckId, (clone $hoy)->modify('-'.(9 + $i).' days')->format('Y-m-d'), max(0, $ultimoKm - 6200), 'Servicio preventivo demo: aceite, filtros y revisión general.']);
        }
    }

    $pdo->prepare("INSERT INTO cv_camioneta_compras (titulo,material,cantidad,proveedor,costo,estatus,notas) VALUES (?,?,?,?,?,?,?)")
        ->execute(['Cambio de filtros preventivo','Filtro de aceite','12 piezas','Proveedor demo',1800,'orden','Compra ficticia para mostrar flujo de compras.']);
    $pdo->prepare("INSERT INTO cv_camioneta_compras (titulo,material,cantidad,proveedor,costo,estatus,notas) VALUES (?,?,?,?,?,?,?)")
        ->execute(['Balatas para unidades','Balatas','4 juegos','Refaccionaria demo',4200,'entrega','Pedido de muestra para dos semanas de operación.']);
}

try {
    cv_demo_init($pdo);
} catch (Throwable $e) {
    $errors[] = 'No se pudieron preparar las tablas demo: ' . $e->getMessage();
}

/* =========================================================
   COLUMNA: IMPORTE DE GASOLINA EN LLEGADA
   En la demo SQLite la columna se crea automáticamente arriba.
========================================================= */
$hasGasolinaLlegadaColumn = true;
$selectGasolinaLlegadaUlt = "COALESCE(ult.gasolina_depositada_llegada, 0)";
$selectGasolinaLlegadaInd = "COALESCE(ind.gasolina_depositada_llegada, 0)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add_truck') {
            ...
            $tanque = min(100, max(0, (float)post('tanque_gasolina', '0')));
            ...
            $stmt = $pdo->prepare("INSERT INTO cv_camionetas (eco,modelo,no_serie,matricula,actividad,fecha_ultimo_mantenimiento,kilometraje_actual,tanque_gasolina) VALUES (?,?,?,?,?,?,?,?)");
            $stmt->execute([$eco,$modelo,$serie,$matricula,$actividad,$mant,$km,$tanque]);
            $truckId = (int)$pdo->lastInsertId();
            $stmt = $pdo->prepare("INSERT INTO cv_camioneta_indicadores_diarios (camioneta_id,fecha,kilometraje,tanque_gasolina) VALUES (?,?,?,?) ON CONFLICT(camioneta_id, fecha) DO UPDATE SET kilometraje=excluded.kilometraje, tanque_gasolina=excluded.tanque_gasolina");
            $stmt->execute([$truckId,$fechaIndicadores,$km,$tanque]);
            if ($nombre !== '') {
                $stmt = $pdo->prepare("INSERT INTO cv_camioneta_conductores (camioneta_id,nombre,telefono,correo,ruta,fecha_inicio,activo) VALUES (?,?,?,?,?,?,1)");
        ...
        if ($action === 'update_plates') {
            ...
            $pdo->prepare("UPDATE cv_camionetas SET matricula=? WHERE id=?")->execute([$mat,$id]); redirect_self();
        }
        ...
        $pdo->prepare("UPDATE cv_camionetas SET no_serie=? WHERE id=?")->execute([$serie, $id]);
        ...
        if ($action === 'update_indicators') {
            ...
            $pdo->prepare("UPDATE cv_camionetas SET kilometraje_actual=?, tanque_gasolina=? WHERE id=?")
                ->execute([$km, $tanque, $id]);

            $pdo->prepare("INSERT INTO cv_camioneta_indicadores_diarios (camioneta_id,fecha,kilometraje,tanque_gasolina,gasolina_depositada) VALUES (?,?,?,?,?) ON CONFLICT(camioneta_id, fecha) DO UPDATE SET kilometraje=excluded.kilometraje, tanque_gasolina=excluded.tanque_gasolina, gasolina_depositada=excluded.gasolina_depositada")
        ...
        if ($action === 'update_arrival') {
            ...
            $gasLlegada = post('tanque_llegada_cedis') !== '' ? min(100, max(0, (float)post('tanque_llegada_cedis'))) : null;
            $importeLlegada = post('gasolina_depositada_llegada') !== '' ? (float)post('gasolina_depositada_llegada') : 0;

            if ($id <= 0) {
                throw new RuntimeException('Camioneta inválida.');
            }

            $stmtActual = $pdo->prepare("SELECT kilometraje_actual, tanque_gasolina FROM cv_camionetas WHERE id=? LIMIT 1");
            ...
            $gasSalida = (float)($actual['tanque_gasolina'] ?? 0);

            if ($hasGasolinaLlegadaColumn) {
                $pdo->prepare("INSERT INTO cv_camioneta_indicadores_diarios (camioneta_id,fecha,kilometraje,tanque_gasolina,kilometraje_llegada,tanque_llegada_cedis,gasolina_depositada_llegada) VALUES (?,?,?,?,?,?,?) ON CONFLICT(camioneta_id, fecha) DO UPDATE SET kilometraje_llegada=excluded.kilometraje_llegada, tanque_llegada_cedis=excluded.tanque_llegada_cedis, gasolina_depositada_llegada=excluded.gasolina_depositada_llegada")
                    ->execute([$id, $fechaIndicadores, $kmSalida, $gasSalida, $kmLlegada, $gasLlegada, $importeLlegada]);
            } else {
                $pdo->prepare("INSERT INTO cv_camioneta_indicadores_diarios (camioneta_id,fecha,kilometraje,tanque_gasolina,kilometraje_llegada,tanque_llegada_cedis) VALUES (?,?,?,?,?,?) ON CONFLICT(camioneta_id, fecha) DO UPDATE SET kilometraje_llegada=excluded.kilometraje_llegada, tanque_llegada_cedis=excluded.tanque_llegada_cedis")
                    ->execute([$id, $fechaIndicadores, $kmSalida, $gasSalida, $kmLlegada, $gasLlegada]);
            }

            if ($kmLlegada !== null) {
                $pdo->prepare("UPDATE cv_camionetas SET kilometraje_actual=? WHERE id=?")
        ...
        if ($action === 'link_route_eco') {
            ...
            $stmtEcoDestino = $pdo->prepare("SELECT id, actividad FROM cv_camionetas WHERE eco=? LIMIT 1");
            ...
            $stmtOrigen = $pdo->prepare("
                SELECT
                    ...
                    cc.fecha_inicio,
                    c.eco AS eco_origen
                FROM cv_camioneta_conductores cc
                INNER JOIN cv_camionetas c ON c.id = cc.camioneta_id
                WHERE cc.activo = 1
                  AND cc.ruta = ?
                ORDER BY cc.id DESC
                LIMIT 1
            ...
            if ($idOrigen === $idDestino) {
                ...
                $pdo->prepare("UPDATE cv_camioneta_conductores SET ruta=? WHERE id=?")
                    ->execute([$ruta, $idConductorOrigen]);

                $pdo->prepare("UPDATE cv_camionetas SET actividad='en ruta' WHERE id=?")
                    ->execute([$idDestino]);
            } else {
                ...
                $pdo->prepare("UPDATE cv_camioneta_conductores SET activo=0, fecha_fin=date(?, '-1 day') WHERE camioneta_id=? AND activo=1")
                    ->execute([$fechaCambio, $idDestino]);

                // 2) Cierra el registro actual del conductor en la ECO origen.
                $pdo->prepare("UPDATE cv_camioneta_conductores SET activo=0, fecha_fin=date(?, '-1 day') WHERE id=?")
                    ->execute([$fechaCambio, $idConductorOrigen]);

                // 3) Crea el mismo conductor en la ECO destino con la misma ruta.
                $pdo->prepare("INSERT INTO cv_camioneta_conductores (camioneta_id,nombre,telefono,correo,ruta,fecha_inicio,activo) VALUES (?,?,?,?,?,?,1)")
                    ->execute([
                        $idDestino,
                ...
                $pdo->prepare("UPDATE cv_camionetas SET actividad='sin uso' WHERE id=?")
                    ->execute([$idOrigen]);
        ...
        if ($action === 'update_driver_route') {
            ...
            $stmt = $pdo->prepare("UPDATE cv_camioneta_conductores SET ruta=? WHERE camioneta_id=? AND activo=1");
        ...
        if ($action === 'update_driver') {
            ...
            $pdo->beginTransaction();
            $pdo->prepare("UPDATE cv_camioneta_conductores SET activo=0, fecha_fin=date(?, '-1 day') WHERE camioneta_id=? AND activo=1")->execute([$fecha,$id]);
            if ($nombre !== '') $pdo->prepare("INSERT INTO cv_camioneta_conductores (camioneta_id,nombre,telefono,correo,ruta,fecha_inicio,activo) VALUES (?,?,?,?,?,?,1)")->execute([$id,$nombre,$tel ?: null,$correo ?: null,$ruta ?: null,$fecha]);
            $pdo->prepare("UPDATE cv_camionetas SET actividad=? WHERE id=?")->execute([$act,$id]);
        ...
        if ($action === 'update_maintenance') {
            ...
            if ($km === null) {
                $stmtKmActual = $pdo->prepare("SELECT kilometraje_actual FROM cv_camionetas WHERE id=?");
                $stmtKmActual->execute([$id]);
                $km = (int)$stmtKmActual->fetchColumn();
            }
            $pdo->prepare("INSERT INTO cv_camioneta_mantenimientos (camioneta_id,fecha_mantenimiento,kilometraje,notas) VALUES (?,?,?,?)")->execute([$id,$fecha,$km,$notas ?: null]);
            $pdo->prepare("UPDATE cv_camionetas SET fecha_ultimo_mantenimiento=?, actividad='mantenimiento' WHERE id=?")->execute([$fecha,$id]);
            redirect_self();
        }
        if ($action === 'skip_maintenance_alert') {
            ...
            $stmtKmActual = $pdo->prepare("SELECT kilometraje_actual FROM cv_camionetas WHERE id=? LIMIT 1");
            $stmtKmActual->execute([$id]);
            ...
            $pdo->prepare("INSERT INTO cv_camioneta_mantenimientos (camioneta_id,fecha_mantenimiento,kilometraje,notas) VALUES (?,?,?,?)")
        ...
        if ($action === 'delete_truck') {
            if (!$canDeleteTruck) {
                throw new RuntimeException('No autorizado. Solo supervisor, administrativo, administrador, gerente o analista puede eliminar cv_camionetas.');
            }
            $id = (int)post('id');
            if ($id <= 0) {
                throw new RuntimeException('Camioneta inválida.');
            ...
            $pdo->prepare("DELETE FROM cv_camionetas WHERE id=?")->execute([$id]);
            redirect_self();
        }
        ...
        if ($action === 'update_purchase_status') {
            ...
            $pdo->prepare("UPDATE cv_camioneta_compras SET estatus=? WHERE id=?")->execute([$estatus, $id]);
            redirect_self();
        }
        if ($action === 'update_purchase') {
            ...
            if ($titulo === '') throw new RuntimeException('Escribe el nombre o motivo de la compra.');
            if ($material === '') throw new RuntimeException('Selecciona el material que se va a comprar.');
            $pdo->prepare("UPDATE cv_camioneta_compras SET titulo=?, material=?, cantidad=?, proveedor=?, costo=?, notas=? WHERE id=?")
        ...
        if ($action === 'delete_purchase') {
            $id = (int)post('id');
            if ($id <= 0) throw new RuntimeException('Compra inválida.');
            $pdo->prepare("DELETE FROM cv_camioneta_compras WHERE id=?")->execute([$id]);
            redirect_self();
        }
...
$vendors = ['Alex Torres','Diego Ruiz','Marco Rivera','Jesus Chuy','Juan Carlos','Santiago','Luis Eduardo','Carmelo','Humberto','Jonathan','Gilberto','Brayan','Felipe','Raul Medina'];
sort($vendors, SORT_NATURAL | SORT_FLAG_CASE);

/* =========================================================
   FILTROS
   - graf_* solo afecta las gráficas superiores.
   - card_* solo afecta las tarjetas de cv_camionetas.
========================================================= */
...
/* =========================================================
   ...
   Sirve para estadísticas generales y tarjetas de cv_camionetas.
========================================================= */
$trucks = $pdo->query("
    SELECT 
        c.*,
        ...
        {$selectGasolinaLlegadaUlt} AS gasolina_depositada_llegada,
        ult.kilometraje_llegada,
        ult.tanque_llegada_cedis
    FROM cv_camionetas c
    LEFT JOIN cv_camioneta_conductores cc 
        ON cc.camioneta_id = c.id 
        AND cc.activo = 1
    LEFT JOIN (
        SELECT i1.*
        FROM cv_camioneta_indicadores_diarios i1
        INNER JOIN (
            SELECT camioneta_id, MAX(fecha) AS fecha_max
            FROM cv_camioneta_indicadores_diarios
    ...
    ORDER BY CASE c.actividad WHEN 'en ruta' THEN 1 WHEN 'sin uso' THEN 2 WHEN 'mantenimiento' THEN 3 ELSE 4 END, CAST(c.eco AS INTEGER)
")->fetchAll();

/* =========================================================
   CONSULTA PARA GRÁFICAS
   Esta consulta SÍ se filtra por fecha y ruta.
   No afecta las tarjetas de cv_camionetas.
========================================================= */
$whereGraf = [];
$paramsGraf = [
...
$sqlGraf = "
    SELECT 
        ...
        {$selectGasolinaLlegadaInd} AS gasolina_depositada_llegada,
        ind.id AS indicador_diario_id,
    ...
    LEFT JOIN cv_camioneta_conductores cc
        ON cc.camioneta_id = c.id
        AND cc.activo = 1
    LEFT JOIN cv_camioneta_indicadores_diarios ind
...
/* =========================================================
   ...
   Solo afecta cv_camionetas activas/inactivas.
...
/* =========================================================
   ...
   Solo revisa cv_camionetas activas/en ruta.
========================================================= */
$fechaSalidaActual = date('Y-m-d');
$salidasPendientes = [];
$llegadasPendientes = [];
$mostrarLlegadasPendientes = (new DateTime('now', new DateTimeZone('America/Monterrey'))) >= new DateTime(date('Y-m-d') . ' 15:30:00', new DateTimeZone('America/Monterrey'));
try {
    ...
    if ($mostrarLlegadasPendientes) {
        $stmtLlegadas = $pdo->prepare("
            SELECT
                c.id,
                c.eco,
                cc.nombre AS conductor,
            ...
            LEFT JOIN cv_camioneta_conductores cc
                ON cc.camioneta_id = c.id
                AND cc.activo = 1
            INNER JOIN cv_camioneta_indicadores_diarios ind
                ON ind.camioneta_id = c.id
                AND ind.fecha = ?
            WHERE c.actividad = 'en ruta'
              AND (
                    ind.kilometraje_llegada IS NULL
                    OR ind.tanque_llegada_cedis IS NULL
                  )
            ORDER BY c.eco
        "
        );
        $stmtLlegadas->execute([$fechaSalidaActual]);
        $llegadasPendientes = $stmtLlegadas->fetchAll();
    }
} catch (Throwable $e) {
    $salidasPendientes = [];
    $llegadasPendientes = [];
...
$kmChart = $trucksGraf;
$gasChart = $trucksGraf;
$gasFaltantes = array_values(array_filter($gasChart, fn($t) => ($t['actividad'] ?? '') === 'en ruta' && empty($t['indicador_diario_id'])));
$gasActualizados = array_values(array_filter($gasChart, fn($t) => !empty($t['indicador_diario_id'])));
...
usort($maintCards, function($a, $b) {
    $aRecienHecho = ((float)($a['status']['recorridos'] ?? 0) <= 0);
    $bRecienHecho = ((float)($b['status']['recorridos'] ?? 0) <= 0);

    if ($aRecienHecho !== $bRecienHecho) {
        return $aRecienHecho <=> $bRecienHecho;
    }

    if ((int)$a['avance'] !== (int)$b['avance']) {
        return (int)$b['avance'] <=> (int)$a['avance'];
    }

    return (int)($a['truck']['eco'] ?? 0) <=> (int)($b['truck']['eco'] ?? 0);
});
...
$compras = [];
try {
    $compras = $pdo->query("SELECT * FROM cv_camioneta_compras ORDER BY CASE estatus WHEN 'orden' THEN 1 WHEN 'pago' THEN 2 WHEN 'entrega' THEN 3 WHEN 'entregado' THEN 4 ELSE 5 END, fecha_creacion DESC LIMIT 100")->fetchAll();
...
$usuarioNav = 'Usuario Demo';
$avatarNav = 'img/LogoGenerico.png';

?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Demo CV | Control de Unidades</title>
<style>
/* =========================================================
...
.nav {
    ...
    align-items: center;
    gap: 28px;
    background: linear-gradient(135deg, #f8fafc 0%, #dbeafe 42%, #a78bfa 100%);
    border: 1px solid rgba(15,23,42,.10);
    border-radius: 26px;
    padding: 12px 24px;
    min-height: 72px;
    color: #0b162e;
    box-shadow: 0 18px 42px rgba(88,28,135,.20);
    position: sticky;
    top: 10px;
    z-index: 5;
}
...
.fuel-card.pending .fuel-top span {
    color: var(--red);
    font-size: 13px;
}
...
/* =========================================================
   BOTÓN DE HISTORIALES EN HEADER
========================================================= */
.hero-date-actions {
    display: flex;
    align-items: center;
...
.show-history-top {
    ...
    border-radius: 14px;
    padding: 12px 18px;
    box-shadow: 0 10px 22px rgba(34,163,99,.22);
}

/* =========================================================
   MODAL GENERAL DE HISTORIALES
========================================================= */
.history-modal-body {
    display: grid;
    grid-template-columns: 1fr;
...
.history-unit-section h4 {
    margin: 0 0 10px;
    font-size: 16px;
}

/* =========================================================
   HISTORIALES: CONDUCTORES
   Evita que la fecha del conductor se pegue al historial diario.
========================================================= */
.history-conductores .history-row {
    display: grid;
    grid-template-columns: 1fr;
    gap: 6px;
}

.history-conductores .history-row strong {
    display: block;
}
...
/* =========================================================
   LINKS DE ALERTAS
   Mantiene el mismo color aunque el enlace ya haya sido visitado.
========================================================= */
.daily-exit-alert a,
.daily-exit-alert a:visited,
.daily-exit-alert a:hover,
.daily-exit-alert a:active,
.daily-arrival-alert a,
.daily-arrival-alert a:visited,
.daily-arrival-alert a:hover,
.daily-arrival-alert a:active {
    color: #003cff;
    font-weight: 950;
}
...
@media(max-width:760px) {
    ...
    /* =========================================================
       MÓVIL: FILTROS Y BOTONES DE CAMIONETAS ACTIVAS
       Se acomodan parejos en una cuadrícula de 2 columnas:
       Ruta actual / Limpiar
       Vincular ruta/ECO / Agregar camioneta
    ========================================================= */
    .section-actions {
        width: 100%;
        display: grid;
...
<body><div class="wrap">
<nav class="nav">
  <img class="logo" src="img/ListonGenerico.png" alt="Listón genérico">
  <a href="#">Cortes</a>
  <a href="#">Administracion</a>
  <a class="active" href="camionetas_cv_demo.php">Consultas</a>
  <a href="#">Rutas</a>
  <div class="hello">
  ...
  <img class="nav-avatar" src="img/LogoGenerico.png" alt="Logo genérico">
</nav>
<section class="hero">
    <h1>Control de Unidades</h1>

    <div class="hero-date-actions">
        <a class="btn green show-history-top" href="#historialesUnidades">Mostrar historiales</a>

        <div class="date">Fecha Actual<br><?php
...
<?php if($salidasPendientes): ?>
    <div class="daily-exit-alert">
        <div class="daily-exit-alert-row">
            <span>⚠️ Falta registrar salida del día actual en estas cv_camionetas activas:</span>
...
<?php if($llegadasPendientes): ?>
    <div class="daily-exit-alert">
        <div class="daily-exit-alert-row">
            <span>⚠️ Después de las 3:30 PM falta registrar llegada al CEDIS en estas cv_camionetas activas:</span>
            <span><?=h(date('d/m/Y', strtotime($fechaSalidaActual)))?></span>
        </div>
        <ul>
            <?php foreach($llegadasPendientes as $lp): ?>
                <li>
                    <a href="#detalle<?=h($lp['id'])?>">
                        ECO <?=h($lp['eco'])?><?=($lp['ruta'] ?? '') !== '' ? ' · ' . h($lp['ruta']) : ''?><?=($lp['conductor'] ?? '') !== '' ? ' · ' . h($lp['conductor']) : ''?>
                    </a>
                </li>
            <?php endforeach; ?>
        </ul>
...
<section class="filter-card compact">
    <form method="get" class="filter-form">
        ...
        <a 
            class="btn light" 
            href="camionetas_cv_demo.php?card_eco=<?=urlencode($cardEco)?>&card_ruta=<?=urlencode($cardRuta)?>"
        >
            Limpiar
        </a>
    </form>
...
<section class="grid">
  ...
  <div class="stat">
    ...
    <div class="fuel-grid scroll-area fuel-scroll">
      <?php if($gasChart): ?>
        <?php if($gasFaltantes): ?>
          <div class="fuel-section-title">Faltantes</div>
          <?php foreach($gasFaltantes as $t): ?>
            <div class="fuel-card pending">
              <div class="fuel-top">
                <strong>ECO <?=h($t['eco'])?></strong>
                <span>Falta actualizar</span>
              </div>
              <div class="progress-track"><div class="progress-fill fuel-fill" style="width:0%"></div></div>
              <div class="fuel-meta">
        ...
        <?php if($gasActualizados): ?>
          <div class="fuel-section-title">Actualizados</div>
          <?php foreach($gasActualizados as $t): ?>
            <div class="fuel-card">
              <div class="fuel-top">
                <strong>ECO <?=h($t['eco'])?></strong>
                <span><?=h((string)($t['tanque_gasolina'] ?? 0))?>%</span>
              </div>
              <div class="progress-track"><div class="progress-fill fuel-fill" style="width:<?=max(0,min(100,(float)($t['tanque_gasolina'] ?? 0)))?>%"></div></div>
              <div class="fuel-meta">
                <span>Depositado salida: $<?=h(number_format((float)($t['gasolina_depositada'] ?? 0),2))?></span>
                <span>Carga llegada: $<?=h(number_format((float)($t['gasolina_depositada_llegada'] ?? 0),2))?></span>
...
<!-- =========================================================
     FILTROS DE TARJETAS
     Solo afectan las tarjetas de cv_camionetas activas/inactivas.
========================================================== -->
<div class="section-head">
    ...
    <div class="section-actions">
        <form method="get" class="cards-filter">
            ...
            <a 
                class="btn light" 
                href="camionetas_cv_demo.php?graf_fecha=<?=urlencode($grafFecha)?>&graf_ruta=<?=urlencode($grafRuta)?>"
            >
                Limpiar
            </a>
        </form>
...
<div class="cards"><?php foreach($active as $t): truck_card($t); endforeach; if(!$active) echo '<p class="hint">Todavía no hay cv_camionetas activas.</p>'; ?></div>
<div class="section-head"><h2>Camionetas Inactivas</h2></div>
<div class="cards"><?php foreach($inactive as $t): truck_card($t); endforeach; if(!$inactive) echo '<p class="hint">Todavía no hay cv_camionetas inactivas.</p>'; ?></div>
...
<div class="truck">
    ...
    <div class="truck-body">
        ...
        <div class="truck-fuel-extra">
            ...
            <div class="truck-fuel-box">
                <span>Carga llegada</span>
                <strong>$<?=h(number_format((float)($t['gasolina_depositada_llegada'] ?? 0), 2))?></strong>
            </div>
        </div>
        ...
        <div class="modal-body">
            ...
            <div class="modal-col">
                    ...
                    <details class="update-panel">
                        ...
                        <form method="post" class="form-grid update-form-grid">
                            ...
                            <label class="field">
                                ...
                                <input name="tanque_gasolina" type="number" min="0" max="100" step="0.01" value="<?=h((string)($t['gasolina_salida_hist'] ?? $t['tanque_gasolina']))?>">
                            </label>
                ...
                <section class="box">
                    ...
                    <div class="trip-data-grid">
                        ...
                        <div class="trip-data-box">
                            ...
                            <p><b>Importe gasolina llegada:</b> $<?=h(number_format((float)($t['gasolina_depositada_llegada'] ?? 0),2))?></p>
                            ...
                            <details class="update-panel">
                                ...
                                <form method="post" class="form-grid update-form-grid">
                                    ...
                                    <label class="field">
                                        ...
                                        <input name="tanque_llegada_cedis" type="number" min="0" max="100" step="0.01" value="<?=h((string)($t['tanque_llegada_cedis'] ?? ''))?>" placeholder="0 a 100">
                                    </label>

                                    <label class="field">
                                        <span>$ gasolina cargada antes de llegar</span>
                                        <input name="gasolina_depositada_llegada" type="number" min="0" step="0.01" value="<?=h((string)($t['gasolina_depositada_llegada'] ?? 0))?>" placeholder="Importe">
                                    </label>
                ...
                <section class="box">
                    ...
                    <?php
                        $mantHist = $pdo->prepare('SELECT * FROM cv_camioneta_mantenimientos WHERE camioneta_id=? ORDER BY fecha_mantenimiento DESC, id DESC LIMIT 20');
            ...
            <div class="modal-col">
                ...
                <section class="box driver-detail-data">
                    ...
                    <?php else: ?>
                        <p class="hint">
                            ...
                            actividad o eliminar cv_camionetas. Los vendedores solo actualizan kilometraje y gasolina.
                        </p>
                    <?php endif; ?>
                ...
                <section class="box">
                    ...
                    <?php
                        $hist = $pdo->prepare('SELECT * FROM cv_camioneta_conductores WHERE camioneta_id=? ORDER BY fecha_inicio DESC');
                        $hist->execute([$t['id']]);
                    ?>
                    ...
                    <?php
                        $ind = $pdo->prepare('SELECT * FROM cv_camioneta_indicadores_diarios WHERE camioneta_id=? ORDER BY fecha DESC LIMIT 15');
                        $ind->execute([$t['id']]);
                    ?>
                    ...
                    <?php foreach($ind as $r): ?>
                        ...
                        <div class="history-row history-row-stacked">
                            ...
                            <strong>
                                ...
                                <?php endif; ?>
                                <?php if((float)($r['gasolina_depositada_llegada'] ?? 0) > 0): ?>
                                    <br>Importe gasolina llegada: $<?=h(number_format((float)$r['gasolina_depositada_llegada'],2))?>
                                <?php endif; ?>
                            </strong>
                        </div>
                    <?php endforeach; ?>
...
<!-- =========================================================
     MODAL GENERAL: SOLO HISTORIALES
========================================================= -->
<div class="modal" id="historialesUnidades">
    <div class="modal-card">
        <div class="modal-head">
            <h2>Historiales de Unidades</h2>
            <a class="close" href="#">×</a>
        </div>

        <div class="history-modal-body">
            <?php foreach($trucks as $histTruck): ?>
                <section class="history-unit-card">
                    <h3>ECO <?=h($histTruck['eco'])?><?=($histTruck['ruta'] ?? '') !== '' ? ' · ' . h($histTruck['ruta']) : ''?></h3>

                    <div class="history-unit-grid">
                        <div class="history-unit-section">
                            <h4>Historial de mantenimientos</h4>
                            <?php
                                $histMant = $pdo->prepare('SELECT * FROM cv_camioneta_mantenimientos WHERE camioneta_id=? ORDER BY fecha_mantenimiento DESC, id DESC LIMIT 8');
                                $histMant->execute([$histTruck['id']]);
                                $hayMant = false;
                            ?>
                            <?php foreach($histMant as $hm): $hayMant = true; ?>
                                <div class="history-row">
                                    <span><?=h($hm['fecha_mantenimiento'])?></span>
                                    <strong><?=h(number_format((float)($hm['kilometraje'] ?? 0)))?> km<?=($hm['notas'] ?? '') !== '' ? '<br><small>' . h($hm['notas']) . '</small>' : ''?></strong>
                                </div>
                            <?php endforeach; ?>
                            <?php if(!$hayMant): ?>
                                <p class="history-empty">Sin mantenimientos registrados.</p>
                            <?php endif; ?>
                        </div>

                        <div class="history-unit-section history-conductores">
                            <h4>Historial de conductores</h4>
                            <?php
                                $histCond = $pdo->prepare('SELECT * FROM cv_camioneta_conductores WHERE camioneta_id=? ORDER BY fecha_inicio DESC, id DESC LIMIT 8');
                                $histCond->execute([$histTruck['id']]);
                                $hayCond = false;
                            ?>
                            <?php foreach($histCond as $hc): $hayCond = true; ?>
                                <div class="history-row">
                                    <span>
                                        <?=h($hc['nombre'])?>
                                        <br><small><?=h($hc['ruta'] ?: 'Sin ruta')?></small>
                                    </span>
                                    <strong><?=h($hc['fecha_inicio'])?> a <?=h($hc['fecha_fin'] ?: 'Actual')?></strong>
                                </div>
                            <?php endforeach; ?>
                            <?php if(!$hayCond): ?>
                                <p class="history-empty">Sin conductores registrados.</p>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="history-unit-section">
                            ...
                            <?php
                                $histInd = $pdo->prepare('SELECT * FROM cv_camioneta_indicadores_diarios WHERE camioneta_id=? ORDER BY fecha DESC, id DESC LIMIT 8');
                                $histInd->execute([$histTruck['id']]);
                                $hayInd = false;
                            ?>
                            <?php foreach($histInd as $hi): $hayInd = true; ?>
                                <?php
                                    $kmSalida = (float)($hi['kilometraje'] ?? 0);
                                    $kmLlegada = ($hi['kilometraje_llegada'] ?? '') !== '' ? (float)$hi['kilometraje_llegada'] : null;
                                    $kmRecorridos = $kmLlegada !== null ? max(0, $kmLlegada - $kmSalida) : null;
                                ?>
                                <div class="history-row history-row-stacked">
                                    <span><?=h($hi['fecha'])?></span>
                                    <strong>
                                        Salida: <?=h(number_format($kmSalida))?> km · <?=h($hi['tanque_gasolina'])?>%
                                        <?php if($kmLlegada !== null): ?>
                                            <br>Llegada: <?=h(number_format($kmLlegada))?> km · Recorridos: <?=h(number_format($kmRecorridos))?> km
                                        <?php endif; ?>
                                        <?php if(($hi['tanque_llegada_cedis'] ?? '') !== ''): ?>
                                            <br>Gasolina llegada: <?=h($hi['tanque_llegada_cedis'])?>%
                                        <?php endif; ?>
                                        <?php if((float)($hi['gasolina_depositada_llegada'] ?? 0) > 0): ?>
                                            <br>Importe gasolina llegada: $<?=h(number_format((float)$hi['gasolina_depositada_llegada'],2))?>
                                        <?php endif; ?>
                                    </strong>
                                </div>
                            <?php endforeach; ?>
                            <?php if(!$hayInd): ?>
                                <p class="history-empty">Sin registros diarios.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
...
(function(){
  ...
  el.textContent = saludo + ' ' + emoji + ' | Demo CV';
